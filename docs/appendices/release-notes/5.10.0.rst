...
- Added caching for public keys from JWK endpoints for JWT authentication.
  By adding the http header ``cache-control: max-age`` the public keys
  once requested will be cached for the given time. See
  :ref:`JWT authentication method <auth_jwt>` for more details.
